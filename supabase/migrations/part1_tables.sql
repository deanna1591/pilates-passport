-- ═══════════════════════════════════════════════════════
-- PILATES PASSPORT — PART 1: TABLES + RLS + TRIGGERS
  ...
  update studios set
    avg_rating = (
      ...
      where studio_id = coalesce(new.studio_id, old.studio_id)
        and moderation_status = 'approved' and is_visible = true
    ),
    review_count = (
...
create policy "Public badge display"
  ...
  using (
    auth.uid() = user_id or
    exists (select 1 from users where users.id = user_badges.user_id and users.visibility = 'public')
  );
